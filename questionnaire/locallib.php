...
//                        $q4 = get_string('notapplicable', 'questionnaire'); DEV JR choose one solution please
    ...
    function response_select($rid, $col = null, $csvexport = false, $choicecodes=0, $choicetext=1) {
        ...
        if ($records = $DB->get_records_sql($sql)) {
            ...
            // --------------------- response_data ---------------------
          // Formdata created here in locallib not questiontypes   FPS BH2F
        $sql = 'SELECT q.id '.$col.',a.choice_id  as aresponse '.
               'FROM {questionnaire_resp_data} a, {questionnaire_question} q '.
               'WHERE a.response_id=\''.$rid.'\' AND a.question_id=q.id ';
        if ($records = $DB->get_records_sql($sql)) {
            foreach ($records as $qid => $record) {
     // Select field id rowcount and value 
	$count = 1;
	$sql = 'SELECT id, recordid as cid ,  fieldid ,content as response from {data_content}  where  recordid = '.$record->aresponse.' ;';
        $rows = $DB->get_records_sql($sql);
        foreach ($rows as $rkey => $row) {
      /* checkboxes stored as sequential content same recordid?  */
		$fid= $row->fieldid;
		$newrow[]= $row->response;
		$values["${qid}_${fid}_${count}"] = $newrow;
	}
		$count++;
                    }
            }
