...
// FPS formdata created here in a wrapper....
                // Multiple.
...
//  FPS  setup formdata
                print_r($arr);
                if ($qtype == QUESDATA) {     // database qs66-3-1.
                        print_r($qid);
                        print_r($qpos);
                        print_r($qname);
                        print_r($val);
    ...
    private function response_select($rid, $col = null, $csvexport = false, $choicecodes=0, $choicetext=1) {
        ...
        // Resp_data. FPS For each recordid in Resp_data there are multiple contents one for each field. 
       // $formdata is constructed here ..
        $mycounter = 0;
        $myrecordid = 0;
        $newrow = array();
        $sql = 'SELECT  c.id as respid,a.choice_id as aresponse , c.fieldid ,f.type,c.content as resp ,q.id'.$col.',q.length,q.precise '.
               ' FROM {questionnaire_resp_data} a, {questionnaire_question} q , {data_content} c , {data_fields} f'.
               ' WHERE a.response_id=\''.$rid.'\' AND a.question_id=q.id  AND c.recordid=a.choice_id AND c.fieldid = f.id';
        if ($records = $DB->get_records_sql($sql)) {
            foreach ($records as $qid => $row) {
                            // print_r($row);
                $survqid = $row->id;
                $recordid = $row->aresponse;
                 if($recordid !== $myrecordid) {
                $myrecordid = $recordid;
		$mycounter +=1;
                }

//  can be lots of rows if added in db mode so ignore this here...
                $noofrows =  $row->length;
                $dataid = $row->precise;
                $myfieldid=$row->fieldid;
                $mycontent=$row->resp;
                switch ($row->type) {
                            case 'menu' :
                                 $suf='s';
               ...
               $values["$suf${survqid}-$myfieldid-$mycounter"] = (array)$mycontent;
                            }
              }
        return($values);
    }
