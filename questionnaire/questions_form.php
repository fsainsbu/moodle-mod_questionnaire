...
class questionnaire_edit_question_form extends moodleform {
    ...
    public function definition() {
        ...
        } else if ($question->type_id == QUESDATA) {
 		/* add dbid mappping here fps select id,name from ch_data */
		$sql = "select id,name from {data} order by name;";
		if (!($resps = $DB->get_records_sql($sql, array()))) {
                            $resps = array();
   $precoptions = array();                    }
$rids = array_keys($resps);
   foreach ($resps as $resp) {
   $precoptions[$resp->id] = $resp->name;
//$mform->addElement('text', 'length', $resp->id, array('size'=>'1'));
// $mform->addElement('text', 'length', $resp->name, array('size'=>'1'));
}

    
/*
            $precoptions = array("4" => get_string('normal', 'questionnaire'),
                                 "1" => get_string('notapplicablecolumn', 'questionnaire'),
                                 "2" => get_string('noduplicates', 'questionnaire'),
                                 "3" => get_string('osgood', 'questionnaire'));
*/
